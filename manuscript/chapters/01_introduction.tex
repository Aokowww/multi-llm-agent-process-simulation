...
arrivals, and observed variants. A data-driven simulator can therefore
be calibrated from the same logs used for process discovery and
...
treats LLM agents as constrained decision modules. They operate inside
...
scenario response. The findings are interpreted conservatively. The
LLM-agent proxy does not dominate the baselines. It is competitive on
control-flow reproduction and obtains the best workforce distance on
both datasets, while the agent-profile and central baselines remain
stronger on several timing and control-flow measures. Under some
LoanApp high-load conditions the proxy responds well, but that advantage
does not survive every stress scenario. This mixed result is the central
point of the study. It shows that LLM-agent process simulation should be
evaluated through both BPS log quality and LLM-agent behavioral quality,
because its value lies in a bounded combination of log reproduction,
scenario response, reasoning traces, and handover observability.
