...
The project follows a design science research logic. The practical
problem is clear: process simulation needs richer resource behavior, but
it still has to remain reproducible and measurable. The designed artifact
is a resource-centric multi-agent simulator with log-derived constraints
and an LLM-style local decision interface. The evaluation compares
generated logs against held-out real logs, using both prototype metrics
and formal BPS quality measures.
...
evaluation, while \(R'\) and \(H'\) expose local reasoning and
