...
The proxy is best understood as a reproducible stand-in for a future
real LLM call. A real LLM module would receive the same structured
state: current case context, previous resource, selected activity,
feasible local resources, historical priors, and memory. It would then
return a selected resource and a rationale. The current proxy implements
this interface without external API calls, making the first experiment
deterministic conditional on the random seed and avoiding prompt drift,
token cost, and invalid natural-language outputs.
...
Several limitations should be acknowledged. First, the current LLM-agent
condition is a deterministic proxy rather than an API-backed LLM. This
was intentional for reproducibility, but it means the study does not yet
measure prompt sensitivity, model drift, invalid textual outputs, or
token cost. Second, the experiment uses one primary dataset.
...
to replace the proxy with an optional real LLM module behind the same
constrained action interface. The second is to add LLM-specific metrics
such as invalid output rate, fallback rate, feasible action-set size,
reason-action consistency, and handover-message consistency. The third
is to evaluate additional logs from the Chapela-Campa artifact,
